...
import { getPatients, getSettings, getTasks } from "@/lib/data";
import {
  addMonths,
...
import { NeedsFollowUpCard } from "./NeedsFollowUpCard";
...
export default async function DashboardPage() {
  ...
  const [allPatients, settings, tasks] = await Promise.all([
  ...
  // Tasks stay private per seller (RLS), so this is already just the viewer's own —
  // used to grey out "Remind me" for a patient that already has one pending.
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <NeedsFollowUpCard items={needsFollowUp} remindedPatientIds={remindedPatientIds} todayIso={todayIso} />
