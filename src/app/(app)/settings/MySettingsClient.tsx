...
const TABS: { id: TabId; label: string; needs: Permission[] | null }[] = [
  { id: "account", label: msg("General"), needs: null },
  { id: "commission", label: msg("Commission & currency"), needs: ["earnings.own"] },
  { id: "cards", label: msg("Cards"), needs: ["earnings.own"] },
];
...
export function MySettingsClient({
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : tr("Something went wrong"));
      }
    });
  }
  ...
  function handleCardsSubmit(formData: FormData) {
    ...
    startCardsTransition(async () => {
      try {
        ...
        setCardsError(e instanceof Error ? e.message : tr("Something went wrong"));
      }
    });
  }
  ...
  function handleEarningsCardsSubmit(formData: FormData) {
    ...
    startEarningsCardsTransition(async () => {
      try {
        ...
        setEarningsCardsError(e instanceof Error ? e.message : tr("Something went wrong"));
      }
    });
  }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-slate-900">{tr("My settings")}</h1>
        <p className="mt-1 text-sm text-slate-500">
          {rich(tr("Your commission and what you see on your screen. Name, email, phone and password are on {link}."), {
            link: (
              <Link href="/profile" className="font-medium text-teal-700 hover:underline">
                {tr("My profile")}
              </Link>
            ),
          })}
      ...
      {activeTab === "account" && (
        <div className="space-y-6">
          ...
          {(has("earnings.own") || has("earnings.all")) && (
            <Card className="p-6">
              <h2 className="mb-1 text-base font-semibold text-slate-900">{tr("Privacy")}</h2>
              <p className="mb-4 text-sm text-slate-500">
                {hidden
                  ? tr("Earnings/commission figures are hidden on Earnings and Patients. Safe to show your screen.")
                  : tr("Earnings/commission figures are visible everywhere. Hide them before sharing your screen.")}
              </p>
              <PrivacyToggleButton />
            </Card>
          )}
          ...
          <Card className="p-6">
            <h2 className="mb-1 text-base font-semibold text-slate-900">{tr("Celebrations")}</h2>
            <p className="mb-4 text-sm text-slate-500">
              {tr("A little confetti (and, if you keep this on, a short chime) when you close a sale, get paid, or hit a higher commission tier.")}
            </p>
      ...
      {activeTab === "commission" && (
        <div className="space-y-6">
          <Card className="p-6">
            <h2 className="mb-1 text-base font-semibold text-slate-900">{tr("Commission tiers")}</h2>
            <p className="mb-5 text-sm text-slate-500">
              {tr("Applied to the full monthly total, not marginal — whichever tier the month's total falls into, that rate applies to the whole total. Plus a fixed payment added every month.")}
            </p>

            <form action={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label>{tr("Tier 1 threshold ({sym}, up to)", { sym })}</Label>
                  <Input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                <div>
                  <Label>{tr("Tier 2 threshold ({sym}, up to)", { sym })}</Label>
                  <Input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                <div>
                  <Label>{tr("Tier 3 rate (%, above tier 2)")}</Label>
                  <Input
                    type="number"
                    step="0.1"
                    min="0"
                ...
                <div>
                  <Label>{tr("Fixed monthly payment ({sym})", { sym })}</Label>
                  <Input
                    type="number"
                    step="0.01"
                    min="0"
              ...
              <p className="text-xs text-slate-500">
                {tr("Amounts, tiers and commission are in the clinic's main currency ({main}).", { main })}
              </p>
              ...
              <div className="flex flex-wrap items-center gap-2 text-sm text-slate-600">
                <label className="flex items-center gap-2">
                  ...
                  />
                  {tr("Also show approx. in")}
                </label>
                <Select name="approx_currency" defaultValue={settings.approx_currency} className="w-auto" aria-label={tr("Approximate currency")}>
                ...
                <span>{tr("alongside earnings figures")}</span>
              </div>
              <p className="text-xs text-slate-400">
                {approx
                  ? tr("Current rate: 1 {main} ≈ {rate} {currency}. Refreshed automatically once a day — approximate, not for invoicing.", {
                      main,
                      rate: approx.rate.toFixed(approx.rate >= 100 ? 2 : 4),
                      currency: approx.currency,
                    })
                  : tr("Rate fetched automatically once a day — approximate, not for invoicing.")}
              </p>

              {error && <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{error}</p>}
              {saved && (
                <p className="rounded-lg bg-emerald-50 px-3 py-2 text-sm text-emerald-700">{tr("Settings saved.")}</p>
              )}

              <div className="flex justify-end pt-2">
                <Button type="submit" disabled={pending}>
                  {pending ? tr("Saving…") : tr("Save settings")}
                </Button>
              </div>
            </form>
          </Card>
          ...
          {settings.show_try && settings.approx_currency !== main && (
            <Card className="p-6">
              <h2 className="mb-1 text-base font-semibold text-slate-900">
                {tr("{pair} rate history", { pair: `${main}/${settings.approx_currency}` })}
              </h2>
              <p className="mb-4 text-sm text-slate-500">
                {tr("Recorded once a day. Approximate — not for invoicing.")}
              </p>
      ...
      {activeTab === "cards" && (
        <div className="space-y-6">
          <Card className="p-6">
            <h2 className="mb-1 text-base font-semibold text-slate-900">{tr("Dashboard cards")}</h2>
            <p className="mb-4 text-sm text-slate-500">
              {tr("Choose which operational stat cards show on the Dashboard, and drag ⠿ to reorder them.")}
            </p>
            <form action={handleCardsSubmit} className="space-y-3">
              ...
              <div className="flex justify-end pt-2">
                <Button type="submit" disabled={cardsPending}>
                  {cardsPending ? tr("Saving…") : tr("Save settings")}
                </Button>
              </div>
            </form>
          </Card>
          ...
          <Card className="p-6">
            <h2 className="mb-1 text-base font-semibold text-slate-900">{tr("Earnings cards")}</h2>
            <p className="mb-4 text-sm text-slate-500">
              {tr("Choose which commission stat cards show on Earnings, and drag ⠿ to reorder them.")}
            </p>
            <form action={handleEarningsCardsSubmit} className="space-y-3">
              ...
              <div className="flex justify-end pt-2">
                <Button type="submit" disabled={earningsCardsPending}>
                  {earningsCardsPending ? tr("Saving…") : tr("Save settings")}
                </Button>
              </div>
            </form>
          </Card>
