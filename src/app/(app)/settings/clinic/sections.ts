...
export type ClinicSectionId = "clinic" | "users" | "roles" | "sales" | "money" | "operations" | "messaging" | "data";
...
export const CLINIC_SECTIONS: ClinicSection[] = [
  ...
  {
    // Temporary home for the users list until step J gives it a Users page.
    id: "users",
    label: "Users",
    description: "Who has an account in this clinic, and their roles.",
    needs: ["team.view", "team.manage"],
  },
  {
    id: "roles",
    label: "Team & roles",
  ...
  },
  {
    ...
    description: "Sellers without an account, and their patients and commission rates.",
    needs: ["sellers.manage"],
  },
  {
    id: "money",
    label: "Money",
...
/** The section Clinic settings opens on: the first one the viewer may see, not counting the
 * temporary Users section (the menu has its own Users entry). */
export function firstClinicSection(permissions: Permission[]): ClinicSection | undefined {
  return visibleClinicSections(permissions).find((s) => s.id !== "users");
}
...
export const SETTINGS_INDEX: { label: string; section: ClinicSectionId; words?: string }[] = [
  ...
  { label: "Users", section: "users", words: "team members accounts invite remove" },
  { label: "Roles", section: "roles", words: "permissions what can do access" },
  { label: "Sellers without an account", section: "sales", words: "seller records patients reassign" },
  { label: "Seller commission rates", section: "sales", words: "tiers percent" },
