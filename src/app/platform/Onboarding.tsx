  ...
  if (complete) {
    return (
      <p className="flex items-center gap-2 text-sm text-emerald-700">
        ...
        Setup complete: all {total} onboarding steps done.
      </p>
    );
  }
        ...
        {steps.map((step) => (
          <li key={step.id} className="flex items-start gap-2.5">
            ...
            <div className="min-w-0">
              <p className={`text-sm ${step.done ? "text-slate-500" : "font-medium text-slate-900"}`}>{step.label}</p>
              {!step.done && <p className="mt-0.5 text-xs text-slate-500">{step.hint}</p>}
            </div>
          </li>
        ))}
      </ol>
