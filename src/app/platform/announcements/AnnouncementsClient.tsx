...
const SECTIONS: { status: AnnouncementStatus; title: string; empty: string }[] = [
  { status: "live", title: msg("Live now"), empty: msg("Nothing is showing to clinics right now.") },
  { status: "scheduled", title: msg("Scheduled"), empty: msg("Nothing scheduled.") },
  { status: "ended", title: msg("Ended"), empty: msg("Nothing yet.") },
];
          ...
          return (
            <Card key={section.status} className="overflow-hidden">
              <div className="flex items-baseline justify-between border-b border-slate-100 px-5 py-3">
                <h2 className="text-base font-semibold text-slate-900">{t(section.title)}</h2>
                <span className="text-xs text-slate-400">{rows.length}</span>
              </div>
              {rows.length === 0 ? (
                <p className="px-5 py-6 text-sm text-slate-400">{t(section.empty)}</p>
              ) : (
...
function AnnouncementRow({ announcement: a, clinicName }: { announcement: Row; clinicName: Map<string, string> }) {
  ...
  async function run(kind: "end" | "delete") {
    if (kind === "delete" && !confirm(t("Delete this announcement? It disappears from the list too."))) return;
    setError(null);
    setPending(kind);
    try {
      if (kind === "end") await endAnnouncement(a.id);
      else await deleteAnnouncement(a.id);
      showToast(kind === "end" ? t("Announcement ended") : t("Announcement deleted"));
      router.refresh();
    } catch (err) {
  ...
  const audience = a.clinicIds
    ? a.clinicIds.map((id) => clinicName.get(id) ?? t("Deleted clinic")).join(", ")
    : t("All clinics");
  ...
  return (
    <li className="px-5 py-4">
      ...
      <p className="mt-2 text-xs text-slate-400">
        {a.status === "scheduled" ? t("Starts {when}", { when: formatActivityTime(a.startsAt) }) : t("Started {when}", { when: formatActivityTime(a.startsAt) })}
        {a.endsAt
          ? ` · ${a.status === "ended" ? t("ended {when}", { when: formatActivityTime(a.endsAt) }) : t("ends {when}", { when: formatActivityTime(a.endsAt) })}`
          : ` · ${t("no end time")}`}
        {a.createdByName && ` · ${t("by {name}", { name: a.createdByName })}`}
      </p>
      {error && <p className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{error}</p>}
      <div className="mt-3 flex gap-2">
        {a.status !== "ended" && (
          <Button type="button" variant="secondary" size="sm" disabled={pending !== null} onClick={() => run("end")}>
            {pending === "end" ? t("Ending…") : a.status === "live" ? t("End now") : t("Cancel")}
          </Button>
        )}
...
function NewAnnouncementForm({ clinics }: { clinics: [string, string][] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("Failed to post announcement"));
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Card className="h-fit p-5">
      <h2 className="text-base font-semibold text-slate-900">{t("New announcement")}</h2>
      <form key={formKey} onSubmit={handleSubmit} className="mt-4 space-y-4">
        <div>
          ...
          <Textarea
            ...
            placeholder={t("e.g. Scheduled maintenance on Sunday 02:00–03:00 (Turkey time). The app may be unavailable briefly.")}
          />
          ...
          <Select name="level" value={level} onChange={(e) => setLevel(e.target.value as AnnouncementLevel)}>
            {ANNOUNCEMENT_LEVELS.map((l) => (
              <option key={l} value={l}>
                {t(ANNOUNCEMENT_LEVEL_LABELS[l])}
                {l === "critical" ? ` (${t("can't be dismissed")})` : ""}
              </option>
            ))}
          </Select>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-400">{t("Blank = right away")}</p>
          </div>
          <div>
            <Label>{t("Ends")}</Label>
            <Input name="ends_at_local" type="datetime-local" />
            <p className="mt-1 text-xs text-slate-400">{t("Blank = until you end it")}</p>
          </div>
        </div>
        ...
        <Button type="submit" disabled={saving || !message.trim()}>
          {saving ? t("Posting…") : t("Post announcement")}
        </Button>
      </form>
    </Card>
  );
