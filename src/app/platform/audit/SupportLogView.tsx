    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        {log.integrity.brokenId === null ? (
          <p className="flex items-center gap-2 text-sm text-emerald-700">
            ...
            Log intact — all {log.integrity.checked} entries verified against the hash chain.
          </p>
        ) : (
          <p className="flex items-center gap-2 text-sm font-medium text-red-700">
            <span className="h-2 w-2 rounded-full bg-red-500" />
            Integrity check failed at entry #{log.integrity.brokenId}: the log was altered from that point.
          </p>
        )}
        <a
        ...
        >
          Export CSV{f.clinicId ? " (this clinic)" : ""}
        </a>
      </div>

      {log.sessions.length === 0 ? (
        <Card className="px-5 py-12 text-center text-sm text-slate-400">No support activity matches these filters.</Card>
      ) : (
        log.sessions.map((s) => (
          <Card key={s.sessionId ?? s.events[0]?.id} className="overflow-hidden">
            <div className="flex flex-wrap items-baseline justify-between gap-2 border-b border-slate-100 px-5 py-3">
              <div>
                ...
                <span className="ml-2 text-sm text-slate-500">by {s.superadminName}</span>
              </div>
            ...
            <ul className="divide-y divide-slate-50">
              {s.events.map((e) => (
                <li key={e.id} className="flex items-start gap-3 px-5 py-2 text-sm">
                  ...
                  <Badge tone={EVENT_TONES[e.event] ?? "slate"}>{EVENT_LABELS[e.event] ?? e.event}</Badge>
      ...
      {log.truncated && (
        <p className="text-xs text-slate-400">
          Showing the latest {SUPPORT_LOG_LIMIT} entries. Narrow the filters, or export, to see everything.
        </p>
      )}
    </div>
  );
