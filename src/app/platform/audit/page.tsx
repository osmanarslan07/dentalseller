...
const TABS = [
  { id: "platform", label: msg("Platform actions") },
  { id: "support", label: msg("Support access") },
] as const;
...
export default async function PlatformAuditPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">{tr("Audit log")}</h1>
        <p className="mt-1 text-sm text-slate-500">
          {tab === "support"
            ? tr("Every support session inside a clinic: what was opened and what was changed. Tamper-evident, exportable on request. Clinics never see this page.")
            : tr("Everything done from the platform area, by every superadmin. Clinics' own activity isn't shown here.")}
        </p>
      </div>

      <nav className="flex gap-1 border-b border-slate-200" aria-label={tr("Audit log sections")}>
        {TABS.map((t) => (
          <Link
            key={t.id}
