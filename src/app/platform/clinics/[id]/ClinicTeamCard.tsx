...
export function ClinicTeamCard({ members }: { members: ClinicMember[] }) {
  ...
  async function handleReset(member: ClinicMember) {
    if (!confirm(t("Reset {name}'s password? Their current password stops working.", { name: member.displayName || member.email || "" }))) return;
    setError(null);
    setCredential(null);
  ...
  return (
    <Card className="overflow-hidden">
      <div className="border-b border-slate-100 p-5">
        <h2 className="text-base font-semibold text-slate-900">{t("Team")}</h2>
        <p className="mt-1 text-xs text-slate-500">{t("Managed by the clinic's own admins from their Settings page.")}</p>
      </div>
          ...
          <li key={member.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <span className="font-medium text-slate-900">{member.displayName || t("Not signed in yet")}</span>
                {member.role === "admin" && <Badge tone="blue">{t("Admin")}</Badge>}
                {!member.isActive && <Badge tone="amber">{t("Deactivated")}</Badge>}
                <LastSeen lastSeenAt={member.lastSeenAt} lastSignInAt={member.lastSignInAt} />
              </div>
              <p className="truncate text-xs text-slate-500">
                {member.email ?? "—"} · {t("joined {date}", { date: formatDate(member.createdAt.slice(0, 10)) })}
              </p>
            </div>
            <Button
              type="button"
            ...
            >
              {pendingId === member.id ? t("Resetting…") : t("Reset password")}
            </Button>
          </li>
        ))}
        {members.length === 0 && <li className="px-5 py-10 text-center text-sm text-slate-400">{t("No accounts yet.")}</li>}
      </ul>
    </Card>
  );
}
