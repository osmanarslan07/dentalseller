...
export default async function PlatformRolesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-slate-900">{t("Role templates")}</h1>
        <p className="mt-1 max-w-3xl text-sm text-slate-500">
          {t("What Sales, Coordinator and Accountant can do by default, in every clinic. Admin always has everything. A clinic that has changed a role in its own Clinic settings → Team & roles keeps its version; the rest follow these at once.")}
        </p>
      </div>
