...
export function PermissionSummary({
  ...
  if (groups.length === 0) return <p className="text-sm text-slate-500">{empty}</p>;

  return (
    <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2">
      {groups.map((g) => (
        <div key={g.label}>
          <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500">{g.label}</dt>
          {g.items.map((p) => {
