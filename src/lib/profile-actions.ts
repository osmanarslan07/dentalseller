...
export async function setMyDisplayName(_prevState: NameState, formData: FormData): Promise<NameState> {
  ...
  if (!name) return { error: await st("Please enter your name") };
  if (name.length > 60) return { error: await st("Name must be 60 characters or fewer") };
...
export async function updateDisplayName(name: string): Promise<void> {
  ...
  if (!trimmed) throw new Error(await st("Please enter your name"));
  if (trimmed.length > 60) throw new Error(await st("Name must be 60 characters or fewer"));
...
export async function changePassword(currentPassword: string, newPassword: string): Promise<void> {
  ...
  if (!user?.email) throw new Error(await st("Not authenticated"));
  if (newPassword.length < 6) throw new Error(await st("New password must be at least 6 characters"));

  const { error: reauthError } = await supabase.auth.signInWithPassword({
    email: user.email,
    password: currentPassword,
  });
  if (reauthError) throw new Error(await st("Current password is incorrect"));
...
export async function updateMyPhone(rawPhone: string): Promise<string | null> {
  ...
  if (error) {
    if (error.code === "23505") throw new Error(await st("Someone else in your clinic already has that phone number"));
...
export async function changeMyEmail(currentPassword: string, rawEmail: string): Promise<string> {
  ...
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error(await st("Enter a valid email address"));
  if (email === user.email.toLowerCase()) throw new Error(await st("That is already your email"));
  ...
  if (!me) throw new Error(await st("Your account isn't active"));
  ...
  if (error) {
    if (/already|registered|exists/i.test(error.message)) throw new Error(await st("Another account already uses that email"));
    throw new Error(error.message);
  }
...
export async function uploadMyAvatar(formData: FormData): Promise<void> {
  ...
  if (!(file instanceof File) || file.size === 0) throw new Error(await st("Choose a photo"));
  if (!AVATAR_TYPES.includes(file.type)) throw new Error(await st("Use a JPG, PNG or WebP image"));
  if (file.size > AVATAR_MAX_BYTES) throw new Error(await st("That photo is too large"));

  const { data: me } = await supabase.from("profiles").select("clinic_id").eq("id", user.id).maybeSingle();
  if (!me?.clinic_id) throw new Error(await st("Your account isn't linked to a clinic"));
